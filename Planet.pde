...
class Planet extends AudioPlanet implements Comparable {
  ...
  final static float SCALE_SPEED=50;
  ...
  //turns out there's a built-in distance function in Processing :)
    return dist(getX(), getY(), getZ(), other.getX(), other.getY(), other.getZ());
  }
